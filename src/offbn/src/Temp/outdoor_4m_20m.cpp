...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){//from start to reaching 2m height
        ...
        if (once && fabs(current_pose.pose.position.z - 4) < 0.05) {
            pose = current_pose;
            ROS_INFO("REACHED 4M");
            move_to(current_pose.pose.position.x + 20, current_pose.pose.position.y, current_pose.pose.position.z, pose, local_pos_pub);
            ROS_INFO("REACHED 20M");
            pose = current_pose;
            once = false;
        }
        local_pos_pub.publish(pose);
