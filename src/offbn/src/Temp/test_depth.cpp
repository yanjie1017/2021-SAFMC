...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){//from start to reaching 2m height
        ...
        if (once && fabs(current_pose.pose.position.z - 0.5) < 0.05) {
            pose = current_pose;
            move_to(0, -1.5, 0.5, pose, local_pos_pub);
            ROS_INFO("START MOVING FORWARD");
            while (depth_val.data > 1) {
            ...
            while(1) {
                if (depth_val.data <= 1) {
                    pose.pose.position.x -= 0.01;
                    ROS_INFO("INSIDE 1.0");
                }else if (depth_val.data > 1.1) {
                    pose.pose.position.x += 0.01;
                    ROS_INFO("OUTSIDE 1.1");
                }else {
                    ROS_INFO("GOOD");
                }
                local_pos_pub.publish(pose);
