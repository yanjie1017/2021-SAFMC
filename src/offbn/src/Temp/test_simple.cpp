...
void move_to(float x, float y, float z, geometry_msgs::PoseStamped& pose, ros::Publisher& local_pos_pub) {
    ...
    while (!((fabs(current_pose.pose.position.x - pose.pose.position.x) < 0.1) && (fabs(current_pose.pose.position.y - pose.pose.position.y) < 0.1) && (fabs(current_pose.pose.position.z - pose.pose.position.z) < 0.1))) {
        local_pos_pub.publish(pose);
        ros::spinOnce();
        rate.sleep();
    }
