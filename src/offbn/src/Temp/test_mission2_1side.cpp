...
bool m2_move (int flag1, int flag2, mavros_msgs::PositionTarget& pos_sp, ros::Publisher& position_setpoint_pub) {
    ...
    int i = 0;
    if (flag1) {
        while (depth_ind.data == "nan"  && i < (2 / INCREMENT + 1) || (depth_ind.data == "NO") && i < (2 / INCREMENT + 1) ) {
            if (depth_ind.data != "nan") {
            switch(flag2) {
            case 1: { //1st side
                pos_sp.position.y -= INCREMENT;
                break;
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){//from start to reaching 2m height
        ...
        if (once && fabs(current_pose.pose.position.z - 1) < 0.05) {
            pose = current_pose;
            mission2_center = current_pose;
            while(depth_val.data <= 0.1 | depth_val.data > 0.5) {
                pose.pose.position.x += INCREMENT;
                local_pos_pub.publish(pose);
                ros::spinOnce();
                rate.sleep();
            ...
            pose = current_pose;
            ROS_INFO("1M POSITION");
            //move_to_raw_without_avoid(current_pose.pose.position.x + 1, current_pose.pose.position.y, 1, pos_sp, position_setpoint_pub);
            //move_to_raw(1, current_pose.pose.position.x, current_pose.pose.position.y - 0.5, 1, pos_sp, position_setpoint_pub);
            if (m2_move(1, 1, pos_sp, position_setpoint_pub)) {
                ROS_INFO("FOUND EDGE");
                move_to_raw_without_avoid(current_pose.pose.position.x, current_pose.pose.position.y - 0.7, 1, pos_sp, position_setpoint_pub);
                ROS_INFO("NOW YAWING");
                do_yaw(PI / 2, pos_sp, position_setpoint_pub);
            }
            pose = current_pose;
